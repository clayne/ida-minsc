...
@utils.multicase(ea=six.integer_types)
@document.parameters(ea='the address to read bytes from')
def read(ea):
...
@document.parameters(ea='the address to read bytes from', size='the number of bytes to read')
def read(ea, size):
...
@document.parameters(bounds='the boundaries to return bytes from')
def read(bounds):
...
class search(object):
    ...
    @document.aliases('search.bytext', 'search.by_string', 'search.bystring')
    @utils.multicase(string=six.string_types)
    @staticmethod
    @utils.string.decorate_arguments('string')
    ...
    @document.aliases('search.byname', 'byname', 'by_name')
    @utils.multicase(name=six.string_types)
    @staticmethod
    @utils.string.decorate_arguments('name')
...
@document.aliases('goof', 'gooffset', 'gotooffset', 'goto_offset')
...
@utils.multicase()
@document.parameters(flags='any number of `idaapi.GN_*` flags to use when fetching the name')
def name(**flags):
    '''Returns the name at the current address.'''
    return name(ui.current.address(), **flags)
@utils.multicase(ea=six.integer_types)
@document.parameters(ea='an address in the database', flags='any number of `idaapi.GN_*` flags to use when fetching the name')
def name(ea, **flags):
...
@document.parameters(string='a string to use as the name', suffix='any other strings to append to the name', flags='any number of `idaapi.SN_*` flags to use when setting the name')
...
@document.parameters(none='the value `None`', flags='any number of `idaapi.SN_*` flags to use when setting the name')
def name(none, **flags):
    '''Removes the name at the current address.'''
    return name(ui.current.address(), none or '', **flags)
@utils.multicase(ea=six.integer_types, string=six.string_types)
@utils.string.decorate_arguments('string', 'suffix')
@document.parameters(ea='an address in the database', string='a string to use as the name', suffix='any other strings to append to the name', flags='any number of `idaapi.SN_*` flags to use when setting the name')
...
@document.parameters(ea='an address in the database', none='the value `None`', flags='any number of `idaapi.SN_*` flags to use when removing the name')
...
@utils.multicase(none=None.__class__)
@document.parameters(none='the value `None`', repeatable='whether the repeatable comment should be removed instead of the non-repeatable one')
...
def comment(ea, string, **repeatable):
    ...
    return res
@utils.multicase(ea=six.integer_types, none=None.__class__)
@document.parameters(ea='an address in the database', none='the value `None`', repeatable='whether the repeatable comment should be removed instead of the non-repeatable one')
...
class address(object):
    ...
    @utils.multicase(ea=six.integer_types)
    @document.parameters(ea='an address in the database to its item address for')
    def __new__(cls, ea):
    ...
    @utils.multicase(ea=six.integer_types)
    @classmethod
    @document.parameters(ea='the address within the database to return an ``interface.bounds_t`` from')
    def bounds(cls, ea):
    ...
    @utils.multicase(bounds=tuple)
    @classmethod
    @document.parameters(bounds='the address boundaries to iterate through')
    ...
    @classmethod
    @document.parameters(bounds='the address boundaries to iterate through', step='a callable that seeks to the next address such as `address.next`')
    ...
    @classmethod
    @utils.multicase(bounds=tuple)
    @document.parameters(bounds='the boundaries of the blocks to return')
    ...
    @document.aliases('address.byoffset', 'byoffset', 'by_offset')
    @classmethod
    @document.parameters(ea='an offset to transform to an address in the database.')
    ...
    @document.aliases('address.getoffset', 'offset', 'getoffset', 'get_offset')
    @utils.multicase()
    @classmethod
    def offset(cls):
    ...
    @utils.multicase(ea=six.integer_types)
    @classmethod
    @document.parameters(ea='an address in the database to transform to an offset.')
    def offset(cls, ea):
...
class type(object):
    ...
    @document.parameters(info='the typeinfo to assign to the address')
    ...
    @document.parameters(ea='an address in the database', info='the ``idaapi.tinfo_t`` to assign to the address')
    ...
    @document.parameters(ea='an address in the database', info='the typeinfo string to assign to the address')
    def __new__(cls, ea, info):
    ...
    @utils.multicase(ea=six.integer_types, size=six.integer_types)
    @staticmethod
    @document.parameters(ea='an address in the database', size='the number of bytes to check')
    ...
    @document.aliases('type.unknownQ', 'type.is_undefined', 'type.undefinedQ', 'is_unknown')
    @utils.multicase()
    @staticmethod
    def is_unknown():
    ...
    @document.aliases('type.labelQ', 'type.nameQ', 'type.has_name')
    @utils.multicase()
    @staticmethod
    def has_label():
    ...
    @document.aliases('type.typeinfoQ', 'type.infoQ')
    @utils.multicase()
    @staticmethod
    def has_typeinfo():
    ...
    @document.aliases('type.structQ', 'type.is_struc', 'type.is_struct')
    @utils.multicase()
    @staticmethod
    def is_structure():
    ...
    @document.aliases('type.is_ref', 'type.is_referenced')
    @utils.multicase()
    @staticmethod
    def is_reference():
    ...
    class array(object):
        ...
        @document.aliases('type.info', 'type.typeinfo')
        @utils.multicase()
        @classmethod
        def element(cls):
...
class xref(object):
    ...
    @utils.multicase(ea=six.integer_types)
    @staticmethod
    @document.parameters(ea='an address containing a data reference')
    def rm_data(ea):
...
class set(object):
    ...
    @classmethod
    @document.parameters(ea='an address in the database', info='the typeinfo to assign to the address')
    ...
    @utils.multicase()
    @classmethod
    @document.parameters(strtype='if ``strtype`` is specified as an integer, then use the integer as the character width of the string. if a tuple is specified, then the second item represents how many bytes to use for the length prefix')
    ...
    @utils.multicase(ea=six.integer_types)
    @classmethod
    @document.parameters(ea='an address in the database', strtype='if ``strtype`` is specified as an integer, then use the integer as the character width of the string. if a tuple is specified, then the second item represents how many bytes to use for the length prefix')
    ...
    @classmethod
    @document.parameters(ea='an address in the database', length='the length of the string', strtype='if ``strtype`` is specified as an integer, then use the integer as the character width of the string. if a tuple is specified, then the second item represents how many bytes to use for the length prefix')
    ...
    class integer(object):
        ...
        @document.aliases('set.integer.byte')
        @utils.multicase()
        @classmethod
        def uint8_t(cls):
        ...
        @document.aliases('set.integer.word')
        @utils.multicase()
        @classmethod
        def uint16_t(cls):
        ...
        @document.aliases('set.integer.dword')
        @utils.multicase()
        @classmethod
        def uint32_t(cls):
        ...
        @document.aliases('set.integer.qword')
        @utils.multicase()
        @classmethod
        def uint64_t(cls):
        ...
        @document.aliases('set.integer.oword')
        @utils.multicase()
        @classmethod
        def uint128_t(cls):
...
class get(object):
    ...
    class integer(object):
        ...
        @document.aliases('get.integer.byte')
        @utils.multicase()
        @classmethod
        @document.parameters(byteorder='if ``byteorder`` is provided, use it to determine the byteorder of the integer')
        ...
        @document.aliases('get.integer.word')
        @utils.multicase()
        @classmethod
        @document.parameters(byteorder='if ``byteorder`` is provided, use it to determine the byteorder of the integer')
        ...
        @document.aliases('get.integer.dword')
        @utils.multicase()
        @classmethod
        @document.parameters(byteorder='if ``byteorder`` is provided, use it to determine the byteorder of the integer')
        ...
        @document.aliases('get.integer.qword')
        @utils.multicase()
        @classmethod
        @document.parameters(byteorder='if ``byteorder`` is provided, use it to determine the byteorder of the integer')
        ...
        @document.aliases('get.integer.oword')
        @utils.multicase()
        @classmethod
        @document.parameters(byteorder='if ``byteorder`` is provided, use it to determine the byteorder of the integer')
